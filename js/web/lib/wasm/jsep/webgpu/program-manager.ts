...
export class ProgramManager {
  ...
  run(buildArtifact: Artifact, inputs: GpuData[], outputs: GpuData[],
    ...
    this.backend.getCommandEncoder();

    if (this.backend.profilingEnabled) {
      // profiling write start timestamp
      this.backend.getCommandEncoder().writeTimestamp(this.backend.profilingQuerySet, 0);
    }
    ...
    if (this.backend.profilingEnabled) {
      ...
      this.backend.getCommandEncoder().writeTimestamp(this.backend.profilingQuerySet, 1);
